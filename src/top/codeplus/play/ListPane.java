...
public class ListPane extends JPanel {
	...
	private JList<PlayListItem> list;
...
//			System.out.println(info.toString());
	...
	public synchronized int getNextIndex() {
		...
		for (i = 0; i < count; i++) {
			PlayListItem item = dataListModel.get(curIndex);
			if (item.available()) {
				repaint();
				return curIndex;
			}
	...
	public synchronized PlayListItem getPlayListItem(int index) {
		return dataListModel.get(index);
	}
	 ...
	 * @return
	 */
	public JList<PlayListItem> getMusicList() {
		return list;
	}
