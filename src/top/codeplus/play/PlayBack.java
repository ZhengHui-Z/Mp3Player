 ...
 *
 * 用构造器  PlayBack(IAudio) 创建一个PlayBack对象；
 * 调用PlayBack对象的  open(String, String)  打开源文件；
 * 
 * 可以调用PlayBack对象的  getHeader()  方法获取 jmp123.decoder.Header 对象；
 * 可以调用PlayBack对象的  getID3Tag()  方法获取 jmp123.decoder.ID3Tag 对象；
 * 
 * 调用PlayBack对象的  start(boolean)  开始播放；
 * 
 * 可调用PlayBack对象的  pause()  方法控制播放暂停或继续；
 * 可调用PlayBack对象的  stop()  方法终止播放；
 * 
 * 播放完一个文件，调用PlayBack对象的  close()  作必要的清理。
 * 
 * 播放从start帧到end的音乐段   start(long, long, JSlider)
 */
public class PlayBack {
	 ...
	 * @throws IOException
	 */
	public boolean  start(long start, long end, JSlider volumeBar) {
		//当前帧的总长度
